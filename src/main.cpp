...
const string strMessageMagic = "bertcoin Signed Message:\n";
...
static const int64 nTargetTimespan = 1 * 8 * 60 * 60; // bertcoin: 1 days
static const int64 nTargetSpacing = 60; // bertcoin: 2 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal bertcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "bertcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1390943624; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1345972;

        if (fTestNet)
        {
            block.nTime    = 1390943624;
            block.nNonce   = 440824;
        }
